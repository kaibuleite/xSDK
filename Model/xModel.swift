...
class xModel: NSObject {
    ...
    // MARK: - 私有变量
    /// 计数器
    private static var x_modelCount = 0
    ...
    required override init() {
        ...
        xModel.x_modelCount += 1
        self.xid = xModel.x_modelCount
        objc_sync_exit(self)
    }
    /// 实例化对象
    /// - Parameter info: 对象信息字典
    /// - Returns: 对象
    public class func x_new(dict : [String : Any]?) -> Self?
    {
    ...
    /// - Returns: 拼接后的结果
    public func x_appending(model : xModel?,
                            isReplaceEmpty : Bool = false) -> Void
    {
        ...
        let list = self.x_getIvarList(obj: obj)
    ...
    /// - Parameter obj: 指定的对象
    /// - Returns: 成员属性列表
    private func x_getIvarList(obj : xModel) -> [String]
    {
        var result : [String] = []
        ...
        while spClass != NSObject.classForCoder() {
            let spList = self.x_getIvarList(objClass: spClass)
            result += spList
        ...
        // 读取自身的成员变量
        let objList = self.x_getIvarList(objClass: obj.classForCoder)
        result += objList
        /* 映射
    ...
    /// - Returns: 成员属性列表
    private func x_getIvarList(objClass : AnyClass) -> [String]
    {
        var result : [String] = []
        var count : UInt32 = 0
    ...
    /// - Parameter obj: 指定的对象
    /// - Returns: 成员属性键值表
    private func x_getStringDictionary(obj : xModel) -> [String : String]
    {
    ...
    {
        let dict = self.x_getStringDictionary(obj: self)
        return dict
    }
}
